...
const SVGButton = (el)=> {
    const btnAry = [];
    const init = (el) => {
        el.addEventListener('mouseover', onOverTabBtn);
        el.addEventListener('mouseout',  onOutTabBtn);

        el.children.forEach((item)=>{
            if(item.id.indexOf('state_') !== -1) {
                btnAry.push(item);
            }
        });
    ...
    const onOverTabBtn = (event) => {
        const target = event.currentTarget;
        tabState('state_2');
    };
    const onOutTabBtn = (event) => {
        tabState('state_1');
    };

    const tabState = (nId) => {
        btnAry.forEach((item)=>{
            if(item.id.indexOf(nId) !== -1) {
                item.style.display = 'block';
            }else{
                item.style.display = 'none';
            }
